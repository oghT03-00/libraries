...
function ExistsFileHashMD5 {
    # TODO: Kiểm tra FileHashMD5 đã được tạo hay chưa
    param (
        [string]$FileHashMD5
    )
    $exists = $false
    try {
        $url = $secrets.CONFIG.Rtdbs[0].rtdb_url;
        if ($null -ne $url) {
            while ($url.ToString().EndsWith('/')) {
                $url = $url.Substring(0, $url.Length - 1)
            }
            $url = $url + "/$FileHashMD5/FileHashMD5.json"
            $response = Invoke-RestMethod $url  -Method 'GET'
            $exists = ($response.ToString() -ne 'null')
        }
    }
    catch {
        $exists = $false
...
function GetLibraryFile ([string] $pathLibrary) {
    ...
    $o.ExistsFileHashMD5 = ExistsFileHashMD5 -FileHashMD5 $o.FileHashMD5
    if ($o.ExistsFileHashMD5 -eq $false) {
        try {
    ...
    if ([System.IO.Directory]::Exists($deploy_libraries )) {
        ...
        foreach ($file in $files) {
            ...
            if ($checkFilename -eq $true) {
                ...
                if ($lib.ExistsFileHashMD5 -eq $false) { 
